# ES-capacity Docker image — build & Vast template setup

One image, three venvs, three independent purposes: **ES training**
(es-at-scale), **verl GRPO training** (an RL baseline to compare against ES),
and **pass@k analysis** (evaluating any checkpoint from either). See
`Dockerfile` for what's in each venv and why they can't share one
environment, and `on_start.sh` for what runs at every instance boot.
...
| Field | Value |
|---|---|
| **Image Path:Tag** | `ghcr.io/Jaysen-Ma/es-capacity:latest` |
...
| **Add recommended volume settings** | Yes — **256GB** `$WORKSPACE` volume (covers verl/vllm checkpoints, base models, ES checkpoints, and eval outputs across all three venvs' storage needs). Confirm after boot with `vast-capabilities \| jq '.instance.workspace_is_volume'` — `on_start.sh` also warns if it's `false`. |
...
es-at-scale's), don't edit `/opt/verl` in place —
